...
import { getErrorMessage, validateQr } from "../../services/bookingsApi";
...
export default function ValidateTicketPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    setResult("");
    try {
      const data = await validateQr(qrData, token);
      setResult(typeof data === "string" ? data : JSON.stringify(data));
    } catch (submitError) {
      setError(getErrorMessage(submitError, "Validation failed."));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <DashboardLayout>
      <div className="mx-auto max-w-3xl rounded-xl border border-white/5 bg-[#16171a] p-8">
        <h1 className="mb-6 flex items-center gap-3 text-2xl font-bold text-white"><ShieldCheck className="h-6 w-6 text-teal-400" />Validate QR Ticket</h1>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            {loading ? <Loader2 className="mx-auto h-4 w-4 animate-spin" /> : "Validate"}
          </button>
        </form>
